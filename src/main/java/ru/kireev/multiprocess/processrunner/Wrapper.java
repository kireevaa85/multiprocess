...
public class Wrapper {
    ...
    private static void simpleWrap() throws IOException {
        logger.info("begin");
        var currentDir = new File("src/main/java");
        Process process = new ProcessBuilder("java", "ru.kireev.multiprocess.processrunner.Job")
                .inheritIO()
    ...
    private static void monitorWrap() throws IOException, InterruptedException {
        logger.info("begin");
        var currentDir = new File("src/main/java");
        var procBuilder = new ProcessBuilder("java", "ru.kireev.multiprocess.processrunner.Job")
                .inheritIO()
                .directory(currentDir);
    ...
    private static void notifyWrap() throws IOException {
        ...
        logger.info("next action 4...");
        logger.info("next action 5...");
